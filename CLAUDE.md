...
- **sub (คำอธิบายใต้ชื่อ):**
  - **Quiz (Ground Lesson Test)** → `N ข้อ` (จำนวนข้อ**จริง** — นับจาก array / การ render จริง ไม่ใช่เลข placeholder ใน HTML ซึ่ง JS เขียนทับ) · ถ้า quiz นั้น**ไม่มีคำตอบให้ (สร้างคำตอบเอง)** ให้เติม ` (คำตอบสร้างเอง)` ต่อท้าย เช่น `40 ข้อ (คำตอบสร้างเอง)`
  - **สรุป (Ground Lesson Summarize)** → **คำแปลภาษาไทยของหัวข้อ (title) เท่านั้น** (ไม่ใส่อย่างอื่น) เช่น title `Meteorology` → sub `อุตุนิยมวิทยา`
